...
class UnsupervisedMetrics(Metric):
    def __init__(self, prefix: str, n_classes: int, extra_clusters: int, compute_hungarian: bool,
                 add_missing_background: bool, match_each_image: bool = False, dist_sync_on_step=True):
        ...
        self.prefix = prefix
        self.add_missing_background = add_missing_background
        self.match_each_image = match_each_image
        self.add_state("stats",
                       default=torch.zeros(n_classes + self.extra_clusters, n_classes, dtype=torch.int64),
                       dist_reduce_fx="sum")
        if add_missing_background or self.extra_clusters == 0:
            self.add_state("histogram",
                           default=torch.zeros(n_classes, n_classes, dtype=torch.float32),
                           dist_reduce_fx="sum")
        else:
            self.add_state("histogram",
                           default=torch.zeros(n_classes+1, n_classes+1, dtype=torch.float32),
                           dist_reduce_fx="sum")

    def update(self, preds: torch.Tensor, target: torch.Tensor):
        if self.match_each_image:
            self.update_each_image(preds, target)
        else:
            self.update_dataset(preds, target)

    def update_dataset(self, preds: torch.Tensor, target: torch.Tensor):

        with torch.no_grad():
            actual = target.reshape(-1)
            preds_ = preds.reshape(-1)
            mask = (actual >= 0) & (actual < self.n_classes) & (preds_ >= 0) & (preds_ < self.n_classes)
            actual = actual[mask]
            preds_ = preds_[mask]
            self.stats += torch.bincount(
                (self.n_classes + self.extra_clusters) * actual + preds_,
                minlength=self.n_classes * (self.n_classes + self.extra_clusters)) \
                .reshape(self.n_classes, self.n_classes + self.extra_clusters).t().to(self.stats.device)

    def update_each_image(self, preds: torch.Tensor, target: torch.Tensor):
        batch_size = preds.size(0)
        with torch.no_grad():
            for i in range(batch_size):
                actual = target[i].reshape(-1)
                preds_ = preds[i].reshape(-1)
                mask = (actual >= 0) & (actual < self.n_classes) & (preds_ >= 0) & (preds_ < self.n_classes)
                actual = actual[mask]
                preds_ = preds_[mask]
                self.stats = torch.bincount(
                    ...
                    minlength=self.n_classes * (self.n_classes + self.extra_clusters)) \
                    .reshape(self.n_classes, self.n_classes + self.extra_clusters).t().to(self.stats.device)
                self.histogram += self.compute_image_hist(self.stats)
    ...
    def compute_image_hist(self, stats):
        if self.compute_hungarian:
            assignments = linear_sum_assignment(stats.detach().cpu(), maximize=True)
            # print(self.assignments)
            if self.extra_clusters == 0:
                histogram = stats[np.argsort(assignments[1]), :]
            elif self.extra_clusters > 0:
                assignments_t = linear_sum_assignment(stats.detach().cpu().t(), maximize=True)
                histogram = stats[assignments_t[1], :]
                missing = list(set(range(self.n_classes + self.extra_clusters)) - set(assignments[0]))
                new_row = stats[missing, :].sum(0, keepdim=True)

                if self.add_missing_background:
                    histogram[:1, :] += new_row
                else:
            ...
            assignments = (torch.arange(self.n_classes).unsqueeze(1),
                                torch.arange(self.n_classes).unsqueeze(1))
            histogram = self.stats

        return histogram

    def compute(self):
        if not self.match_each_image:
            if self.compute_hungarian:
                ...
                if self.extra_clusters == 0:
                    ...
                    self.assignments_t = linear_sum_assignment(self.stats.detach().cpu().t(), maximize=True)
                    histogram = self.stats[:, self.assignments_t[0]]
                    self.histogram = histogram[self.assignments_t[1], :]
                    # missing = list(set(range(self.n_classes)) - set(self.assignments[0]))
                    # self.stats[:, missing] = 0
                    # new_row = self.stats[:, missing].sum(1, keepdim=True)
                    # histogram = torch.cat([histogram, new_row], axis=0)
                    # new_col = torch.zeros(self.n_classes + 1, 1, device=histogram.device)
                    # self.histogram = torch.cat([histogram, new_col], axis=1)
            else:
